...
import admsgenter.convenient_equivalent.mixin.EMCMappingHandlerAccessor;
...
import admsgenter.convenient_storage.lib.函数.消耗;
...
public class 转化功能 extends 数据功能 implements 面板数据, 配置功能, 外部存储<物品键, ItemStack> {
    ...
    private static final 数据包类型.一<byte[]> 同步转化 = 频道.创建(处理器.主线程((数据, 信息) -> {
        ...
        if(信息.player().containerMenu instanceof TransmutationContainer 菜单) 菜单.transmutationInventory.updateClientTargets(true);
    ...
    private static final 数据包类型.一<ItemInfo> 学习知识 = 频道.创建(处理器.主线程((数据, 信息) -> {
        if((数据.A.getItem() == PEItems.TOME_OF_KNOWLEDGE.asItem()? 转化功能.页面.get().学习全部():转化功能.页面.get().学习(数据.A)) &&
            信息.player().containerMenu instanceof TransmutationContainer 菜单) 菜单.transmutationInventory.itemLearned(数据.A);
    ...
    private static final 数据包类型.一<ItemInfo> 遗忘知识 = 频道.创建(处理器.主线程((数据, 信息) -> {
        if((数据.A.getItem() == PEItems.TOME_OF_KNOWLEDGE.asItem()? 转化功能.页面.get().遗忘全部():转化功能.页面.get().遗忘(数据.A)) &&
            信息.player().containerMenu instanceof TransmutationContainer 菜单) 菜单.transmutationInventory.itemUnlearned(数据.A);
    }), ItemInfo.STREAM_CODEC);
    public final Set<ItemInfo> 存储集 = 检索表.O(80), 无效集 = 检索表.O(10);
    ...
    public 转化功能(Tag 数据, 面板空间 空间) {
        ...
        if(数据 instanceof CompoundTag 等价) {
            ...
            Consumer<ItemInfo> 条件 = 消耗.条件(判断.add(存储集), Predicates.compose(Predicates.equalTo(Items.AIR.builtInRegistryHolder()), ItemInfo::getItem));
            for(Tag 标签 : 等价.getList("knowledge", Tag.TAG_COMPOUND)) ItemInfo.CODEC.parse(NbtOps.INSTANCE, 标签).ifSuccess(条件);
            充能组 = 读取物品(等价.get("charge"));
            放能组 = 读取物品(等价.get("discharge"));
            遗忘组 = 读取物品(等价.get("unlearn"));
            启用 = 等价.contains("enabled");
            全部 = 等价.contains("full_knowledge");
        }
        空间.物品().添加存储(this, 8);
    }

    public 转化功能(RegistryFriendlyByteBuf 数据, 面板空间 空间) {
        super(空间);
        EMC.设置(ByteBufCodecs.BYTE_ARRAY.decode(数据));
        int 数量 = 数据.readVarInt();
        while(--数量 >= 0) 存储集.add(ItemInfo.STREAM_CODEC.decode(数据));
        全部 = 数据.readBoolean();
        空间.物品().添加存储(this, 8);
    }
    ...
    public boolean 学习全部() {
        if(全部) return false;
        ItemInfo 知识之书 = ItemInfo.fromItem((Holder<Item>) PEItems.TOME_OF_KNOWLEDGE);
    ...
    @Override
    public 复合存储器 序列化() {
        ListTag 数据 = new ListTag();
        for(ItemInfo 信息 : 存储集) ItemInfo.CODEC.encodeStart(NbtOps.INSTANCE, 信息).ifSuccess(判断.add(数据));
        RegistryAccess 注册 = 注册();
        return new 复合存储器().列表("transmutationEmc", new IntArrayTag(EMC.值())).列表("knowledge", 数据)
            .物品组("charge", 注册, 充能组).物品组("discharge", 注册, 放能组).物品组("unlearn", 注册, 遗忘组).逻辑("enabled", 启用).逻辑("full_knowledge", 全部);
    }

    @Override
    public void 序列化(RegistryFriendlyByteBuf 数据) {
        ByteBufCodecs.BYTE_ARRAY.encode(数据, EMC.字节());
        数据.writeVarInt(存储集.size());
        for(ItemInfo 信息 : 存储集) ItemInfo.STREAM_CODEC.encode(数据, 信息);
        数据.writeBoolean(全部);
    }
    ...
    @Override
    public void 写入(RegistryFriendlyByteBuf 数据包) { 数据包.writeBoolean(启用); }

    @Override
    public void 读取(RegistryFriendlyByteBuf 数据包) { 启用 = 数据包.readBoolean(); }
    ...
    public void setItem(int 索引, ItemStack 物品组) {
        switch(索引) {
            ...
            case 2 -> {
                ...
                IItemEmcHolder 电池 = 物品组.getCapability(PECapabilities.EMC_HOLDER_ITEM_CAPABILITY);
                if(电池 != null) {
    ...
    private static ItemInfo 信息(物品键 键) { return ItemInfo.fromItem(键.物品(), 键.组件()); }

    private static boolean 有效(物品键 键) { return 键.空组件() || 键.组件().size() == 1 && 键.组件().get(DataComponents.DAMAGE) != null; }
